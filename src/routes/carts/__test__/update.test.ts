...
describe('Database has valid cart entries', () => {
	...
	test('Return 400 on invalid update contents', async () => {
		await request(app)
			.patch(`${testRoute}/${cartId}`)
			.send('Some invalid update')
			.expect(400);
	});
	test('Return 404 on valid contents but non existing cart id request', async () => {
		await request(app)
			.patch(`${testRoute}/${createMongooseId()}`)
			.send(update)
			.expect(404);
	});
	...
	test('Updates expiration date', async () => {
		const { body: cart } = await request(app)
			...
			.send(update)
			.expect(200);
		const prevDate = new Date(cartExpiresAt);
		const currentDate = new Date(cart.expiresAt);
